defmodule Advent3 do
    ...
    def part2_next_element({num, x, y}, map) do
        Enum.sum(
            for x_offset <- -1..1, y_offset <- -1..1, {:ok, value} <- [Map.fetch(map, {x + x_offset, y + y_offset})] do 
                value
            end
        )
    end
...
Advent3.generate(368078, &Advent3.part1_next_element/2)
Advent3.generate(368078, &Advent3.part2_next_element/2)