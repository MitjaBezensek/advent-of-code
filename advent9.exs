defmodule Advent9 do
  def count(input) do
    ...
    removed_garbage =
      no_exclamations
      |> Enum.reduce(%{garbage_started: false, result: [], garbage_count: 0}, &remove_garbage/2)

    IO.inspect(count_groups(removed_garbage.result).sum)
    IO.inspect(Map.fetch!(removed_garbage, :garbage_count))
  end

  def remove_garbage("<", %{garbage_started: false} = acc),
    do: Map.put(acc, :garbage_started, true)

  def remove_garbage(">", acc), do: Map.put(acc, :garbage_started, false)

  def remove_garbage(_, %{garbage_started: true} = acc),
    do: Map.put(acc, :garbage_count, acc.garbage_count + 1)
